...
/* Master variables */
volatile uint16 IMU_I2C_mstrStatus;      /* Master Status byte  */
volatile uint8  IMU_I2C_mstrControl;     /* Master Control byte */

/* Receive buffer variables */
volatile uint8 * IMU_I2C_mstrRdBufPtr;   /* Pointer to Master Read buffer */
volatile uint32  IMU_I2C_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  IMU_I2C_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * IMU_I2C_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  IMU_I2C_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  IMU_I2C_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  IMU_I2C_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!IMU_I2C_CY_SCBIP_V0 && \
    IMU_I2C_I2C_MULTI_MASTER_SLAVE_CONST && IMU_I2C_I2C_WAKE_ENABLE_CONST)
    static void IMU_I2C_I2CMasterDisableEcAm(void);
#else
    #define IMU_I2C_I2CMasterDisableEcAm()     do{ /* Empty */ }while(0)
#endif /* (!IMU_I2C_CY_SCBIP_V0) */

static uint32 IMU_I2C_I2CMasterHandleStatus(uint32 status);
static uint32 IMU_I2C_I2CMasterWaitOneUnit(uint32 *timeout);


/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterWriteBuf
****************************************************************************//**
*
...
*  Enables the I2C interrupt and clears IMU_I2C_I2C_MSTAT_WR_CMPLT
*  status.
*
...
*  - IMU_I2C_I2C_MODE_COMPLETE_XFER - Perform complete transfer
*    from Start to Stop.
*  - IMU_I2C_I2C_MODE_REPEAT_START - Send Repeat Start instead
*    of Start. A Stop is generated after transfer is completed unless
*    NO_STOP is specified.
*  - IMU_I2C_I2C_MODE_NO_STOP Execute transfer without a Stop.
*    The following transfer expected to perform ReStart.
*
* \return
*  Error status.
*  - IMU_I2C_I2C_MSTR_NO_ERROR - Function complete without error.
*    The master started the transfer.
*  - IMU_I2C_I2C_MSTR_BUS_BUSY - Bus is busy. Nothing was sent on
*    the bus. The attempt has to be retried.
*  - IMU_I2C_I2C_MSTR_NOT_READY - Master is not ready for to start
...
*
* \globalvars
*  IMU_I2C_mstrStatus  - used to store current status of I2C Master.
*  IMU_I2C_state       - used to store current state of software FSM.
*  IMU_I2C_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  IMU_I2C_mstrWrBufPtr - used to store pointer to master write buffer.
*  IMU_I2C_mstrWrBufIndex - used to current index within master write
*  buffer.
*  IMU_I2C_mstrWrBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 IMU_I2C_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = IMU_I2C_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(IMU_I2C_CHECK_I2C_FSM_IDLE)
        {
            IMU_I2C_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = IMU_I2C_CHECK_I2C_STATUS(IMU_I2C_I2C_STATUS_BUS_BUSY) ?
                            IMU_I2C_I2C_MSTR_BUS_BUSY : IMU_I2C_I2C_MSTR_NO_ERROR;
        }
        else if(IMU_I2C_CHECK_I2C_FSM_HALT)
        {
            IMU_I2C_mstrStatus &= (uint16) ~IMU_I2C_I2C_MSTAT_XFER_HALT;
                              errStatus  = IMU_I2C_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(IMU_I2C_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        /* Non-empty for master-slave mode when wakeup enabled */
        IMU_I2C_I2CMasterDisableEcAm();

        /* Set up write transaction */
        IMU_I2C_state = IMU_I2C_I2C_FSM_MSTR_WR_ADDR;
        IMU_I2C_mstrWrBufIndexTmp = 0u;
        IMU_I2C_mstrWrBufIndex    = 0u;
        IMU_I2C_mstrWrBufSize     = cnt;
        IMU_I2C_mstrWrBufPtr      = (volatile uint8 *) wrData;
        IMU_I2C_mstrControl       = (uint8) mode;

        slaveAddress = IMU_I2C_GET_I2C_8BIT_ADDRESS(slaveAddress);

        IMU_I2C_mstrStatus &= (uint16) ~IMU_I2C_I2C_MSTAT_WR_CMPLT;

        IMU_I2C_ClearTxInterruptSource    (IMU_I2C_INTR_TX_UNDERFLOW);
        IMU_I2C_ClearMasterInterruptSource(IMU_I2C_INTR_MASTER_ALL);
        IMU_I2C_SetMasterInterruptMode    (IMU_I2C_I2C_INTR_MASTER_MASK);
        ...
        if(IMU_I2C_CHECK_I2C_MODE_RESTART(mode))
        {
            IMU_I2C_I2C_MASTER_GENERATE_RESTART;
            IMU_I2C_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            IMU_I2C_TX_FIFO_WR_REG = slaveAddress;
            IMU_I2C_I2C_MASTER_GENERATE_START;
        }
    }

    IMU_I2C_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterReadBuf
****************************************************************************//**
*
*  Automatically reads an entire buffer of data from a slave device.
*  Once the data transfer is initiated by this function, further data transfer
*  is handled by the included ISR.
*  Enables the I2C interrupt and clears IMU_I2C_I2C_MSTAT_RD_CMPLT
*  status.
*
*  \param slaveAddr: 7-bit slave address.
*  \param xferData: Pointer to buffer of data to be sent.
...
*  be ORed together. See IMU_I2C_I2CMasterWriteBuf()
*  function for constants.
*
* \return
*  Error status.See IMU_I2C_I2CMasterWriteBuf()
*  function for constants.
*
* \globalvars
*  IMU_I2C_mstrStatus  - used to store current status of I2C Master.
...
*  IMU_I2C_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  IMU_I2C_mstrRdBufPtr - used to store pointer to master read buffer.
*  IMU_I2C_mstrRdBufIndex - used to current index within master read
*  buffer.
*  IMU_I2C_mstrRdBufSize - used to store master read buffer size.
*
*******************************************************************************/
uint32 IMU_I2C_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(IMU_I2C_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        IMU_I2C_state = IMU_I2C_I2C_FSM_MSTR_RD_ADDR;
        IMU_I2C_mstrRdBufIndex = 0u;
        IMU_I2C_mstrRdBufSize  = cnt;
        IMU_I2C_mstrRdBufPtr   = (volatile uint8 *) rdData;
        IMU_I2C_mstrControl    = (uint8) mode;

        slaveAddress = (IMU_I2C_GET_I2C_8BIT_ADDRESS(slaveAddress) | IMU_I2C_I2C_READ_FLAG);

        IMU_I2C_mstrStatus &= (uint16) ~IMU_I2C_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(IMU_I2C_mstrRdBufSize < IMU_I2C_I2C_FIFO_SIZE)
        {
        ...
        else
        {
            ...
            IMU_I2C_ENABLE_MASTER_AUTO_DATA_ACK;
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterSendStart
****************************************************************************//**
*
...
* address byte.
*  - IMU_I2C_I2C_WRITE_XFER_MODE - Set write direction for the
*    following transfer.
*  - IMU_I2C_I2C_READ_XFER_MODE - Set read direction for the
*    following transfer.
*
* \param timeoutMs
...
* Error status.
*  - IMU_I2C_I2C_MSTR_NO_ERROR - Function complete without error.
*  - IMU_I2C_I2C_MSTR_BUS_BUSY - Bus is busy.
*    Nothing was sent on the bus. The attempt has to be retried.
*  - IMU_I2C_I2C_MSTR_NOT_READY - Master is not ready for to
*    start transfer.
...
*    Nothing was sent on the bus. The attempt has to be retried.
*  - IMU_I2C_I2C_MSTR_ERR_LB_NAK - Error condition: Last byte was
*    NAKed.
*  - IMU_I2C_I2C_MSTR_ERR_ARB_LOST - Error condition: Master lost
*    arbitration.
*  - IMU_I2C_I2C_MSTR_ERR_BUS_ERR - Error condition: Master
*    encountered a bus error. Bus error is misplaced start or stop detection.
*  - IMU_I2C_I2C_MSTR_ERR_ABORT_START - Error condition: The start
...
{
    uint32 errStatus = IMU_I2C_I2C_MSTR_NOT_READY;

    if(IMU_I2C_CHECK_I2C_FSM_IDLE)
    {
        IMU_I2C_DisableInt();  /* Lock from interruption */

        if(IMU_I2C_CHECK_I2C_STATUS(IMU_I2C_I2C_STATUS_BUS_BUSY))
        {
            errStatus = IMU_I2C_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            uint32 locStatus;
            uint32 timeout = IMU_I2C_I2C_CONVERT_TIMEOUT_TO_US(timeoutMs);

            /* Non-empty for master-slave mode when wakeup enabled */
            IMU_I2C_I2CMasterDisableEcAm();

            /* Clean up the hardware before a transaction. */
            IMU_I2C_SetMasterInterruptMode    (IMU_I2C_NO_INTR_SOURCES);
            IMU_I2C_ClearMasterInterruptSource(IMU_I2C_INTR_MASTER_ALL);
            ...
            else             /* Read direction */
            {
                IMU_I2C_state = IMU_I2C_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= IMU_I2C_I2C_READ_FLAG;
            }
            ...
            do
            {
                locStatus  = IMU_I2C_GetMasterInterruptSource() & IMU_I2C_INTR_MASTER_SEND_BYTE;
                locStatus |= IMU_I2C_GetSlaveInterruptSource() & IMU_I2C_SLAVE_INTR_I2C_ADDR;
                locStatus |= IMU_I2C_I2CMasterWaitOneUnit(&timeout);
            }
            while (0u == locStatus);

            /* Handle completion event and convert to status */
            errStatus = IMU_I2C_I2CMasterHandleStatus(locStatus);
        }
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterSendRestart
****************************************************************************//**
*
...
* \param bitRnW
* Direction of the following transfer. It is defined by read/write bit within
* address byte.See IMU_I2C_I2CMasterSendStart() function for constants.
*
* \param timeoutMs
* Defines in milliseconds the time that this function can block for.
* If that time expires, the function returns. If a zero is passed,
...
* \return
* Error status.
* See IMU_I2C_I2CMasterSendStart() function for constants.
*
* \sideeffect
...
uint32 IMU_I2C_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW, uint32 timeoutMs)
{
    ...
    if(IMU_I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        uint32 locStatus;
        uint32 timeout = IMU_I2C_I2C_CONVERT_TIMEOUT_TO_US(timeoutMs);
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterSendStop
****************************************************************************//**
*
...
uint32 IMU_I2C_I2CMasterSendStop(uint32 timeoutMs)
{
    ...
    if(IMU_I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = IMU_I2C_GetMasterInterruptSource() & IMU_I2C_INTR_MASTER_SEND_STOP;
            locStatus |= IMU_I2C_I2CMasterWaitOneUnit(&timeout);
        }
        while (0u == locStatus);
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterWriteByte
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterReadByte
****************************************************************************//**
*
...
* IMU_I2C_I2CMasterSendStop() or
* IMU_I2C_I2CMasterSendRestart() function appropriately.
* This function is blocking. It does not return until a byte is received or
* an error occurs.
*
* \param ackNack
* Response to received byte.
*  - IMU_I2C_I2C_ACK_DATA - Generates ACK.
*     The master notifies slave that transfer continues.
*  - IMU_I2C_I2C_NAK_DATA - Prepares to generate NAK.
...
uint32 IMU_I2C_I2CMasterReadByte(uint32 ackNack, uint8 *rdByte, uint32 timeoutMs)
{
    ...
    if(IMU_I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = IMU_I2C_GetMasterInterruptSource() & IMU_I2C_INTR_MASTER_RECEIVE_BYTE;
            rxNotEmpty = IMU_I2C_GetRxInterruptSource() & IMU_I2C_INTR_RX_NOT_EMPTY;
            locStatus |= IMU_I2C_I2CMasterWaitOneUnit(&timeout);
        }
        ...
        if (0u != rxNotEmpty)
        {
            *rdByte = (uint8)IMU_I2C_RX_FIFO_RD_REG;
            IMU_I2C_ClearRxInterruptSource(IMU_I2C_INTR_RX_ALL);
        }
        ...
        if (IMU_I2C_I2C_MSTR_NO_ERROR == errStatus)
        {
            if(IMU_I2C_I2C_ACK_DATA == ackNack)
            {
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes that has been transferred with an
*  IMU_I2C_I2CMasterReadBuf() function.
*
* \return
...
*  IMU_I2C_I2C_MSTAT_ERR_ARB_LOST or
*  IMU_I2C_I2C_MSTAT_ERR_BUS_ERROR occurred while read transfer.
*
* \globalvars
*  IMU_I2C_mstrRdBufIndex - used to current index within master read
*  buffer.
...
{
    return(IMU_I2C_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterGetWriteBufSize
****************************************************************************//**
*
...
{
    return(IMU_I2C_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterClearWriteBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterStatus
****************************************************************************//**
*
...
*  - IMU_I2C_I2C_MSTAT_RD_CMPLT - Read transfer complete.
*    The error condition status bits must be checked to ensure that
*    read transfer was completed successfully.
*  - IMU_I2C_I2C_MSTAT_WR_CMPLT - Write transfer complete.
*    The error condition status bits must be checked to ensure that write
*    transfer was completed successfully.
*  - IMU_I2C_I2C_MSTAT_XFER_INP - Transfer in progress.
*  - IMU_I2C_I2C_MSTAT_XFER_HALT - Transfer has been halted.
*    The I2C bus is waiting for ReStart or Stop condition generation.
*  - IMU_I2C_I2C_MSTAT_ERR_SHORT_XFER - Error condition: Write
*    transfer completed before all bytes were transferred. The slave NAKed
*    the byte which was expected to be ACKed.
*  - IMU_I2C_I2C_MSTAT_ERR_ADDR_NAK - Error condition: Slave did
*    not acknowledge address.
*  - IMU_I2C_I2C_MSTAT_ERR_ARB_LOST - Error condition: Master lost
*    arbitration during communications with slave.
*  - IMU_I2C_I2C_MSTAT_ERR_BUS_ERROR - Error condition: bus error
*    occurred during master transfer due to misplaced Start or Stop
*    condition on the bus.
*  - IMU_I2C_I2C_MSTAT_ERR_ABORT_XFER - Error condition: Slave was
...
*  - IMU_I2C_I2C_MSTAT_ERR_XFER - Error condition: This is the
...
uint32 IMU_I2C_I2CMasterStatus(void)
{
    ...
    status = (uint32) IMU_I2C_mstrStatus;

    if (IMU_I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) IMU_I2C_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterClearStatus
****************************************************************************//**
*
*  Clears all status flags and returns the master status.
*
* \return
*  Current status of master. See the IMU_I2C_I2CMasterStatus()
*  function for constants.
*
* \globalvars
*  IMU_I2C_mstrStatus - used to store current status of I2C Master.
...
uint32 IMU_I2C_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) IMU_I2C_mstrStatus;
    IMU_I2C_mstrStatus = IMU_I2C_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CReStartGeneration
****************************************************************************//**
*
...
*
*******************************************************************************/
void IMU_I2C_I2CReStartGeneration(void)
{
#if(IMU_I2C_CY_SCBIP_V0)
    /* Generates Restart use GPIO and scb IP functionality. Ticket ID#143715,
    * ID#145238 and ID#173656.
    * In case of timeout the IMU_I2C_I2CMasterSendRestart handles this
    * case using own timeout and execute appropriate recovery.
    */

    const uint16 oneUnit = 1u;
    uint32 timeout = IMU_I2C_I2C_PHASE_GEN_TIMEOUT;

    /* Store direction of transfer */
    uint32 readDirection = IMU_I2C_I2C_STATUS_REG & IMU_I2C_I2C_STATUS_M_READ;

    /* Wait until slave releases SDA line: SCL tHIGH is complete */
    while ((IMU_I2C_WAIT_SDA_SET_HIGH) && (timeout > 0u))
    {
        CyDelayUs(oneUnit);
        --timeout;
    }
    ...
    {
        /* Refill timeout */
        timeout = IMU_I2C_I2C_PHASE_GEN_TIMEOUT;

        /* Drive SCL line use GPIO */
        IMU_I2C_SET_I2C_SCL_DR       (IMU_I2C_I2C_SCL_LOW);
        IMU_I2C_SET_I2C_SCL_HSIOM_SEL(IMU_I2C_HSIOM_GPIO_SEL);
        ...
        if (0u != readDirection)
        {
            ...
            IMU_I2C_SET_I2C_SCL_DR(IMU_I2C_I2C_SCL_LOW);
            CyDelayUs(IMU_I2C_I2C_TLOW_TIME);

            /* Drive tHIGH: SCL high phase */
            IMU_I2C_SET_I2C_SCL_DR(IMU_I2C_I2C_SCL_HIGH);
            CyDelayUs(IMU_I2C_I2C_THIGH_TIME);

            /* Wait until slave releases SDA line: SCL tHIGH is complete */
            while ((IMU_I2C_WAIT_SCL_SET_HIGH) && (timeout > 0u))
            {
                CyDelayUs(oneUnit);
                --timeout;
            }
        ...
        if (timeout > 0u)
        {
            ...
            IMU_I2C_I2C_MASTER_CMD_REG  = IMU_I2C_I2C_MASTER_CMD_M_START;
        }
        ...
        */
        ...
        IMU_I2C_SET_I2C_SCL_HSIOM_SEL(IMU_I2C_HSIOM_I2C_SEL);
        IMU_I2C_SET_I2C_SCL_DR       (IMU_I2C_I2C_SCL_HIGH);
    }
#else
    uint32 cmd;
    ...
    cmd  = IMU_I2C_I2C_MASTER_CMD_M_START;
    cmd |= IMU_I2C_CHECK_I2C_STATUS(IMU_I2C_I2C_STATUS_M_READ) ?
                (IMU_I2C_I2C_MASTER_CMD_M_NACK) : (0u);

    IMU_I2C_I2C_MASTER_CMD_REG = cmd;
    ...
    IMU_I2C_I2C_MULTI_MASTER_SLAVE_CONST && IMU_I2C_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: IMU_I2C_I2CMasterDisableEcAm
    ****************************************************************************//**
    *
    ...
    static void IMU_I2C_I2CMasterDisableEcAm(void)
    {
        ...
        */
        if (0u != (IMU_I2C_CTRL_REG & IMU_I2C_CTRL_EC_AM_MODE))
        {
            ...
            IMU_I2C_Stop();
            IMU_I2C_CTRL_REG &= (uint32) ~IMU_I2C_CTRL_EC_AM_MODE;
            IMU_I2C_Enable();
        }
    }
#endif /* (!IMU_I2C_CY_SCBIP_V0) */
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CMasterHandleStatus
****************************************************************************//**
*
...
* IMU_I2C_I2C_MASTER_TIMEOUT and IMU_I2C_SLAVE_INTR_I2C_ADDR.
*
* \return
* Error status.
* See IMU_I2C_I2CMasterSendStart() function for constants.
...
static uint32 IMU_I2C_I2CMasterHandleStatus(uint32 status)
{
    ...
    if (0u != (status & IMU_I2C_I2C_MASTER_TIMEOUT))
    {
        retStatus  = IMU_I2C_I2C_MASTER_TIMEOUT;
        resetBlock = IMU_I2C_I2C_RESET_ERROR;
    }
    else if (0u != (IMU_I2C_SLAVE_INTR_I2C_ADDR & status))
    {
        /* Abort the master operation, the slave was addressed first */
        retStatus = IMU_I2C_I2C_MSTR_ERR_ABORT_START;

        IMU_I2C_I2C_MASTER_CMD_REG = 0u;

        IMU_I2C_state = IMU_I2C_I2C_FSM_IDLE;
    }
    else if (0u != (status & IMU_I2C_INTR_MASTER_I2C_ARB_LOST))
    {
        retStatus  = IMU_I2C_I2C_MSTR_ERR_ARB_LOST;

        if (IMU_I2C_CHECK_I2C_FSM_ADDR)
        {
            /* If slave is not enabled reset IP, otherwise give it a chance to match address */
            if (0u == (IMU_I2C_I2C_CTRL_REG & IMU_I2C_I2C_CTRL_SLAVE_MODE))
            {
                resetBlock = IMU_I2C_I2C_RESET_ERROR;
            }
    ...
    else if (0u != (status & IMU_I2C_INTR_MASTER_I2C_BUS_ERROR))
    {
        retStatus  = IMU_I2C_I2C_MSTR_ERR_BUS_ERR;
        resetBlock = IMU_I2C_I2C_RESET_ERROR;
    }
    else if (0u != (status & IMU_I2C_INTR_MASTER_I2C_NACK))
    {
        retStatus = IMU_I2C_I2C_MSTR_ERR_LB_NAK;
    }
    else /* Includes: INTR_MASTER_I2C_STOP and INTR_MASTER_I2C_ACK */
    {
        retStatus = IMU_I2C_I2C_MSTR_NO_ERROR;

        if (0u != (status & IMU_I2C_INTR_MASTER_I2C_STOP))
        {
        ...
        else
        {
            ...
            if (IMU_I2C_CHECK_I2C_FSM_ADDR)
            {
                ...
                IMU_I2C_state = (IMU_I2C_CHECK_I2C_FSM_RD) ?
                                          IMU_I2C_I2C_FSM_MSTR_RD_DATA : IMU_I2C_I2C_FSM_MSTR_WR_DATA;
            }
        }
    }

    if (IMU_I2C_I2C_RESET_ERROR == resetBlock)
    {
...
static uint32 IMU_I2C_I2CMasterWaitOneUnit(uint32 *timeout)
{
    ...
    if (*timeout > 0u)
    {
        ...
        if (0u == *timeout)
        {
            status = IMU_I2C_I2C_MASTER_TIMEOUT;
        }
    }
