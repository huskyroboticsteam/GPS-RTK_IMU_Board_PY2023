...
volatile uint8 IMU_I2C_slStatus;          /* Slave Status */

/* Receive buffer variables */
volatile uint8 * IMU_I2C_slWrBufPtr;      /* Pointer to Receive buffer  */
volatile uint32  IMU_I2C_slWrBufSize;     /* Slave Receive buffer size  */
volatile uint32  IMU_I2C_slWrBufIndex;    /* Slave Receive buffer Index */

/* Transmit buffer variables */
volatile uint8 * IMU_I2C_slRdBufPtr;      /* Pointer to Transmit buffer  */
volatile uint32  IMU_I2C_slRdBufSize;     /* Slave Transmit buffer size  */
volatile uint32  IMU_I2C_slRdBufIndex;    /* Slave Transmit buffer Index */
volatile uint32  IMU_I2C_slRdBufIndexTmp; /* Slave Transmit buffer Index Tmp */
volatile uint8   IMU_I2C_slOverFlowCount; /* Slave Transmit Overflow counter */

#if (IMU_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST)

    uint32 (*IMU_I2C_customAddressHandler)(void) = NULL;

    /*******************************************************************************
    * Function Name: IMU_I2C_SetI2cAddressCustomInterruptHandler
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    void IMU_I2C_SetI2cAddressCustomInterruptHandler(uint32 (*func) (void))
    {
        ...
        IMU_I2C_customAddressHandler = func;
    }
#endif /* (IMU_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */


/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveStatus
****************************************************************************//**
*
...
*   - IMU_I2C_I2C_SSTAT_RD_CMPLT - Slave read transfer complete. 
...
*   - IMU_I2C_I2C_SSTAT_RD_BUSY - Slave read transfer is in progress. 
*     Set when master addresses slave with a read, cleared when 
*    IMU_I2C_I2C_SSTAT_RD_CMPLT is set.
*   - IMU_I2C_I2C_SSTAT_RD_OVFL - Master attempted to read more bytes
*     than are in buffer. Slave continually returns 0xFF byte in this case.
*   - IMU_I2C_I2C_SSTAT_RD_ERR - Slave captured error on the bus during 
*     a read transfer. The sources of error are: misplaced Start or Stop 
*     condition or lost arbitration while slave drives SDA.
*   - IMU_I2C_I2C_SSTAT_WR_CMPLT - Slave write transfer complete. 
...
*     successfully.
*   - IMU_I2C_I2C_SSTAT_WR_BUSY - Slave write transfer is in progress. 
*     Set when the master addresses the slave with a write, cleared when 
*    IMU_I2C_I2C_SSTAT_WR_CMPLT is set.
*   - IMU_I2C_I2C_SSTAT_WR_OVFL - Master attempted to write past end of 
*     buffer. Further bytes are ignored.
*   - IMU_I2C_I2C_SSTAT_WR_ERR - Slave captured error on the bus during
...
*     IMU_I2C_I2C_SSTAT_WR_ERR is set. It is recommended to discard write 
...
{
    return((uint32) IMU_I2C_slStatus);
}


/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveClearReadStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear IMU_I2C_I2C_SSTAT_RD_BUSY.
*
* \globalvars
*  IMU_I2C_slStatus  - used to store current status of I2C slave.
*
...
uint32 IMU_I2C_I2CSlaveClearReadStatus(void)
{
    ...
    status = ((uint32) IMU_I2C_slStatus & IMU_I2C_I2C_SSTAT_RD_MASK);
    IMU_I2C_slStatus &= (uint8) ~IMU_I2C_I2C_SSTAT_RD_CLEAR;
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveClearWriteStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear IMU_I2C_I2C_SSTAT_WR_BUSY.*
*
* \globalvars
*  IMU_I2C_slStatus  - used to store current status of I2C slave.
*
...
uint32 IMU_I2C_I2CSlaveClearWriteStatus(void)
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = ((uint32) IMU_I2C_slStatus & IMU_I2C_I2C_SSTAT_WR_MASK);
    IMU_I2C_slStatus &= (uint8) ~IMU_I2C_I2C_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveSetAddress
****************************************************************************//**
*
...
void IMU_I2C_I2CSlaveSetAddress(uint32 address)
{
    ...
    matchReg = IMU_I2C_RX_MATCH_REG;

    matchReg &= ((uint32) ~IMU_I2C_RX_MATCH_ADDR_MASK); /* Clear address bits */
    matchReg |= ((uint32)  IMU_I2C_GET_I2C_8BIT_ADDRESS(address));

    IMU_I2C_RX_MATCH_REG = matchReg;
}


/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveSetAddressMask
****************************************************************************//**
*
...
void IMU_I2C_I2CSlaveSetAddressMask(uint32 addressMask)
{
    ...
    matchReg &= ((uint32) ~IMU_I2C_RX_MATCH_MASK_MASK);
    matchReg |= ((uint32) (addressMask << IMU_I2C_RX_MATCH_MASK_POS));
...
*
* \globalvars
*  IMU_I2C_slRdBufPtr   - used to store pointer to slave read buffer.
*  IMU_I2C_slRdBufSize  - used to store salve read buffer size.
*  IMU_I2C_slRdBufIndex - used to store current index within slave
*  read buffer.
*
*******************************************************************************/
void IMU_I2C_I2CSlaveInitReadBuf(uint8 * rdBuf, uint32 bufSize)
{
    ...
    if(NULL != rdBuf)
    {
        ...
        IMU_I2C_slRdBufPtr      = (volatile uint8 *) rdBuf; /* Set buffer pointer  */
        IMU_I2C_slRdBufSize     = bufSize;                  /* Set buffer size     */
        IMU_I2C_slRdBufIndex    = 0u;                       /* Clear buffer index  */
        IMU_I2C_slRdBufIndexTmp = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveInitWriteBuf
****************************************************************************//**
*
*  Sets the buffer pointer and size of the write buffer. This function also 
*  resets the transfer count returned with the 
*  IMU_I2C_I2CSlaveGetWriteBufSize() function.
*
...
*
* \globalvars
*  IMU_I2C_slWrBufPtr   - used to store pointer to slave write buffer.
*  IMU_I2C_slWrBufSize  - used to store salve write buffer size.
...
void IMU_I2C_I2CSlaveInitWriteBuf(uint8 * wrBuf, uint32 bufSize)
{
    ...
    if(NULL != wrBuf)
    {
        ...
        IMU_I2C_slWrBufPtr   = (volatile uint8 *) wrBuf; /* Set buffer pointer  */
        IMU_I2C_slWrBufSize  = bufSize;                  /* Set buffer size     */
        IMU_I2C_slWrBufIndex = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes read by the I2C master since an
*  IMU_I2C_I2CSlaveInitReadBuf() or 
*  IMU_I2C_I2CSlaveClearReadBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if IMU_I2C_I2C_SSTAT_RD_ERR was 
...
{
    return(IMU_I2C_slRdBufIndex);
}


/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveGetWriteBufSize
****************************************************************************//**
*
*  Returns the number of bytes written by the I2C master since an
*  IMU_I2C_I2CSlaveInitWriteBuf() or 
*  IMU_I2C_I2CSlaveClearWriteBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if IMU_I2C_I2C_SSTAT_WR_ERR was 
*  captured by the slave.
*
* \globalvars
*  IMU_I2C_slWrBufIndex - used to store current index within slave
...
{
    return(IMU_I2C_slWrBufIndex);
}


/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: IMU_I2C_I2CSlaveClearWriteBuf
****************************************************************************//**
*
